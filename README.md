...
> [!IMPORTANT]
> On every step, if you have a [custom domain](https://auth0.com/docs/customize/custom-domains), replace the `YOUR_AUTH0_DOMAIN` and `dev-w1gpyeme4j7v0z15.us.auth0.com` placeholders with your custom domain instead of the value from the settings page.
...
Rename `.env.example` to `.env` and replace the `LDb2msUoePzo4oGuK5l6hdGh1G7DFlZW` and `dev-w1gpyeme4j7v0z15.us.auth0.com` placeholders with the Client ID and domain of your Auth0 application.

```sh
AUTH0_DOMAIN=dev-w1gpyeme4j7v0z15.us.auth0.com
AUTH0_CLIENT_ID=LDb2msUoePzo4oGuK5l6hdGh1G7DFlZW
AUTH0_CUSTOM_SCHEME=com.auth0.sample # For Android
```

### Android: Configure the string resources

In the sample, we are using values referenced from `android/app/src/main/res/values/strings.xml`. Rename `strings.xml.example` to `strings.xml` and replace the `dev-w1gpyeme4j7v0z15.us.auth0.com` placeholder with the domain of your Auth0 application.

```xml
<?xml version="1.0" encoding="utf-8"?>
<resources>
    <string name="com_auth0_domain">dev-w1gpyeme4j7v0z15.us.auth0.com</string>
